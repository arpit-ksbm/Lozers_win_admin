...
import { get_contest, create_contest, update_contest, delete_contest } from '../../utils/api-routes';
import { Color } from '../../assets/color';

function* getContest() {
    try {
        yield put({ type: actionTypes.SET_IS_LOADING, payload: true });
        const { data } = yield call(getAPI, get_contest);
        console.log('Get Contest Saga Response ::: ', data);

        if (data?.success) {
            yield put({ type: actionTypes.SET_CONTEST, payload: data?.contests });
        }
...
export default function* contestSaga() {
    yield takeLeading(actionTypes.GET_CONTEST, getContest);

...
const FunSoratTimer = () => {
    ...
    const { fstGameHistoryData } = useSelector(state => state?.gameHistoryReducer);

    const [searchText, setSearchText] = useState('');
    const filteredData = DeepSearchSpace(fstGameHistoryData, searchText);
    ...
    useEffect(() => {
        //! Dispatch API For Get FST History
        dispatch(GameHistoryActions.getFSTGameHistory());
    }, []);

    return (
        <>
            <RouteHeader title={`Fun Sorat Timer`} />
            <Box sx={{ p: 4 }}>
                <div style={{ background: "#ffffff", padding: "25px", BiBorderRadius: "10px", boxshadow: "0 3px 6px rgba(0, 0, 0, 0.16), 0 0px 0px rgba(0, 0, 0, 0.23)" }}>
                    <DatatableHeading title={'Fun Sorat Timer'} data={fstGameHistoryData} />

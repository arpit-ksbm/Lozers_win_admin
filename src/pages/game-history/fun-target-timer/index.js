...
const FunTargetTimer = () => {
    ...
    const { fttGameHistoryData } = useSelector(state => state?.gameHistoryReducer);

    const [searchText, setSearchText] = useState('');
    const filteredData = DeepSearchSpace(fttGameHistoryData, searchText);
    ...
    const columns = [
        ...
        { name: 'Multiply By', selector: row => row?.totalBetAmountMultipliedBy9 },
    ];

    useEffect(() => {
        //! Dispatch API For Get FTT History
        dispatch(GameHistoryActions.getFTTGameHistory());
    }, []);

    return (
        <>
            <RouteHeader title={`Fun Target Timer`} />
            <Box sx={{ p: 4 }}>
                <div style={{ background: "#ffffff", padding: "25px", BiBorderRadius: "10px", boxshadow: "0 3px 6px rgba(0, 0, 0, 0.16), 0 0px 0px rgba(0, 0, 0, 0.23)" }}>
                    <DatatableHeading title={'Fun Target Timer'} data={fttGameHistoryData} />
